...
func (r *historyRepository) Get(req *dto.Pagination) (RepositoryResult, int) {
	var historys []entity.Subscribe
	...
	// Hitung total data
	if errCount := r.DB.Model(&entity.Subscribe{}).Where("detail_id = ?", req.DetailID).Count(&totalRows).Error; errCount != nil {
		return RepositoryResult{Error: errCount}, totalPages
	}

	for i := range historys {
		historys[i].OrderID = helpers.TruncateString(historys[i].OrderID, 47)
	}
